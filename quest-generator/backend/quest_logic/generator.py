...
def create_quest_from_setting(setting_text: str, api_key: str):
    """
    Генерирует структуру квеста на основе текстового сеттинга,
    используя Groq API.
    """
    client = Groq(api_key=api_key)

    # Мастер-промпт, который инструктирует модель
    master_prompt = f"""
    ...
    Правила:
    1.  JSON должен быть валидным и строго следовать структуре, описанной ниже.
    2.  Квест должен иметь как минимум 3-4 узла (nodes).
    ...
    5.  Описание ('description') и текст выбора ('text') должны быть
        креативными и соответствовать сеттингу.
    ...
    Теперь сгенерируй JSON для этого квеста.
    """
