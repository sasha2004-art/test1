...
*   **A8: Непрерывная проверка.** Всегда запускайте скрипт верификации `start.py` перед созданием коммита.
...
1.  **Локальный Запуск Тестов:** Запустите тесты, которые вы написали или изменили, используя `pytest tests/<ваш_модуль_тестов>.py`. Убедитесь, что они проходят успешно.
2.  **Полная Верификация Проекта:** Запустите скрипт `start.py` он автоматически запустить скрипт тестирования. **Это обязательный шаг.**
3.  **Анализ Вывода:** Внимательно проанализируйте весь вывод скрипта `verify.sh`.
    *   Прошли ли Black, Flake8, Pyright, Pip-audit, Bandit, Semgrep?
    *   Все ли тесты Pytest пройдены?
    *   Достигнуто ли покрытие кода не менее 90%?
4.  **Ручная Проверка (при необходимости):** Если изменения затрагивают пользовательский интерфейс или API, выполните ручную проверку в браузере или через клиент API (`http://localhost:5001`).
...
1.  **Финальная Верификация:** **Обязательно** запустите `start.py` еще раз, чтобы убедиться, что все изменения (включая возможный рефакторинг) не нарушили ни одного правила и все тесты проходят.
...
1.  **Docker** весь проект следует запускать и тестировать в Docker-контейнере. Это обеспечивает изоляцию и воспроизводимость среды. перед работой убедитесь, что у вас установлен Docker и Docker Compose.
2.  **Запустите среду:** Проект полностью контейнеризован. Используйте `python start.py` для запуска. Веб-интерфейс будет доступен по адресу `http://localhost:5001`.
